...
var storedInputEvents : Dictionary = {}
...
func _ready():
	...
	var openSignal = KeybindsAPI.register_keybind({
	  "action_name": "toggle_node_explorer",
	  "title": "Toggle NodeExplorer",
	  "key": KEY_F10,
	})
	KeybindsAPI.connect(openSignal, self, "onPressed")
...
func disableInputEvents():
	...
	for action in InputMap.get_actions():
		if action == "toggle_node_explorer": continue
...
func _node_tree_add(node: Node):
	...
	var nodeInstanceID = nodeParent.get_instance_id()
	if not sceneDisplayerDictionary.has(nodeInstanceID): return
	var parentTreeItem: TreeItem = sceneDisplayerDictionary[nodeInstanceID]
...
func _node_tree_remove(node: Node):
	...
	if sceneDisplayerDictionary.has(nodeId):
		...
		if (!erased):
			print("Tried to remove a node that is not present in the dictionary")
		treeItem.free()
